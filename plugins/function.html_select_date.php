 ...
 * @ignore
 */
require_once(__DIR__ . '/shared.make_timestamp.php');
...
function smarty_function_html_select_date($params)
{
        ...
        switch ($_key) {
            case 'time':
                if (!is_array($_value) && $_value !== null) {
                    $time = fenom_make_timestamp($_value);
                }
                break;
            ...
            default:
                if (!is_array($_value)) {
                    $extra_attrs .= ' ' . $_key . '="' . Fenom\Smarty\Util::escape($_value) . '"';
                } else {
    ...
    if ($display_years) {
        ...
        if ($year_as_text) {
            ...
            if ($year_id !== null || $all_id !== null) {
                $_html_years .= ' id="' . Fenom\Smarty\Util::escape(
    ...
    if ($display_months) {
        ...
        if ($month_id !== null || $all_id !== null) {
            $_html_months .= ' id="' . Fenom\Smarty\Util::escape(
        ...
        for ($i = 1; $i <= 12; $i ++) {
            $_val = sprintf('%02d', $i);
            $_text = isset($month_names) ? Fenom\Smarty\Util::escape($month_names[$i]) : ($month_format == "%m" ? $_val : strftime($month_format, $_month_timestamps[$i]));
    ...
    if ($display_days) {
        ...
        if ($day_id !== null || $all_id !== null) {
            $_html_days .= ' id="' . Fenom\Smarty\Util::escape(
